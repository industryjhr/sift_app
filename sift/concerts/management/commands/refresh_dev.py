...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        try:
            ...
            concerts_fixtures = os.listdir(FIXTURE_DIRS['venues'])
        ...
        concert_fixture_name = datetime.utcnow().strftime('%Y%m%d') + '_concert.json' # CONST?
        concert_fixture_dir = os.path.join(FIXTURE_DIRS['concerts'], concert_fixture_name)
        self.stdout.write("Dumping new concerts to {}".format(concert_fixture_name))
        call_command(
            'dumpdata',
            'concerts.Concert',
            '--indent=4',
            '--output={}'.format(concert_fixture_dir)
        )
